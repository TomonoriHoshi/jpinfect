...
#' `prefecture`, `year`, and `week` as fixed identifiers. If the dataset has more than 5 columns,
#' it is pivoted to a long format.
#'
#' @param dataset A data frame containing the columns `prefecture`, `year`, `week`,
...
#' If `prefecture`, `year`, or `week` are missing from the dataset, the function stops with an error.
#'
#' @details
...
#' - The function ensures that the columns `prefecture`, `year`, and `week` are always present.
#'
#' @examples
#' \dontrun{
#' # Pivot the dataset
...
jpinfect_pivot <- function(dataset) {
  ...
  required_columns <- c("prefecture", "year", "week")
  ...
  if (col_count <= 5) {
    temp_data <- dataset %>% pivot_wider(id_cols = c(prefecture, year, week), names_from = disease, values_from = cases)
  } else {
    temp_data <- dataset %>% pivot_longer(cols = -c(prefecture, year, week), names_to = "disease", values_to = "cases")
  }
  return(temp_data)
